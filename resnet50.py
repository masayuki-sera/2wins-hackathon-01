...
class EarlyStopping:
    """バリデーション損失に基づく早期終了"""
    def __init__(self, patience=5, verbose=False, path="checkpoints/resnet50_best.pth"):
...
if __name__ == "__main__":
    ...
    for fold, (train_idx, val_idx) in enumerate(skf.split(np.zeros(len(labels)), labels), 1):
        ...
        model = ResNet50Modified(num_classes=len(LABELS)).to(device)
        criterion = nn.CrossEntropyLoss()
        optimizer = optim.Adam(model.parameters(), lr=0.001)

        best_path = f"checkpoints/resnet50_best_fold{fold}.pth"
